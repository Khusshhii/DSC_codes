...
struct Tree *Delete(struct Tree *root,char word[])                             // Deleting the word from BST
{
	struct Tree *store;
	if(strcmp(root->arr,word) < 0)
...
int search(struct Tree *root,char word[])                                      // Searching 
{
	if(root == NULL)
		return 0;
	else if(strcmp(root->arr,word) == 0)
...
void del_from_file(char word[])                                                // Deleting from file
{
	...
	fclose(fp2);
	remove("meanings.txt");
	rename("Temporary.txt","meanings.txt");
}
